...
public class FingerprintCardFeeOnlyLetterTX {

	private static final Logger log = LoggerFactory.getLogger(FingerprintCardFeeOnlyLetterTX.class);
	...
	private static void writePdf(TrackingRecordScreeningLetter screeningLetter, OutputStream ba, HttpServletRequest request) throws DocumentException, BadElementException, IOException {
        ...
        paragraph = getParagraph();
        paragraph.add(new Phrase("The applicant submitted fingerprint cards previously. We have one fingerprint card on file that we can ", mediumfont));
        ...
        paragraph.add(new Phrase("the payment references the applicant's name and last four digits of the applicant's social security number.", mediumfont));
        paragraph.setSpacingBefore(10);
        document.add(paragraph);

        paragraph = getParagraph();
        paragraph.add(new Phrase("Failure to return the fee within 5 working days will result in the background screening application being denied, and the ", mediumfont));
        paragraph.add(new Phrase("applicant will not be eligible to be associated with the licensed program in any capacity or will not be eligible to proceed ", mediumfont));
